...
namespace {/**
 * Nette Framework (version 2.1-dev released on 2013-03-14, http://nette.org)
 *
 ...
 */
@header('Content-Type: text/html; charset=utf-8');define('NETTE',TRUE);define('NETTE_DIR',__DIR__);define('NETTE_VERSION_ID',20100);define('NETTE_PACKAGE','5.3');}namespace Nette\Diagnostics{use
Nette;interface
IBarPanel{function
getTab();function
...
getBelongsToReference($table,$key);}}namespace Nette\Forms{use
Nette;interface
IControl{function
...
function
extensionMethod($name,$callback=NULL){if(strpos($name,'::')===FALSE){$class=get_called_class();}else{list($class,$name)=explode('::',$name);}if($callback===NULL){return
...
Nette\Utils\Html::el('a')->href(strtr(Debugger::$editor,array('%file'=>rawurlencode($file),'%line'=>$line)))->title("$file:$line")->setHtml(htmlSpecialChars(rtrim($dir,DIRECTORY_SEPARATOR)).DIRECTORY_SEPARATOR.'<b>'.htmlSpecialChars(basename($file)).'</b>'.($line?":$line":''));}else{return
...
Dumper::toText($var);}}}namespace Nette\Utils{use
Nette;class
Html
extends
Nette\Object
...
__METHOD__."() error: Email address must be a string.\n";exit(254);}self::$email=$email;}if(!self::$enabled){register_shutdown_function(array(__CLASS__,'_shutdownHandler'));set_exception_handler(array(__CLASS__,'_exceptionHandler'));set_error_handler(array(__CLASS__,'_errorHandler'));self::$enabled=TRUE;}}static
function
isEnabled(){return
self::$enabled;}static
function
...
instanceof
Nette\FatalErrorException?'Fatal error: '.$exception->getMessage():get_class($exception).": ".$exception->getMessage())." in ".$exception->getFile().":".$exception->getLine();$hash=md5($exception);$exceptionFilename="exception-".@date('Y-m-d-H-i-s')."-$hash.html";foreach(new\DirectoryIterator(self::$logDirectory)as$entry){if(strpos($entry,$hash)){$exceptionFilename=$entry;$saved=TRUE;break;}}}elseif(!is_string($message)){$message=Dumper::toText($message);}if($exceptionFilename){$exceptionFilename=self::$logDirectory.'/'.$exceptionFilename;if(empty($saved)&&$logHandle=@fopen($exceptionFilename,'w')){ob_start();ob_start(function($buffer)use($logHandle){fwrite($logHandle,$buffer);},4096);self::$blueScreen->render($exception);ob_end_flush();ob_end_clean();fclose($logHandle);}}self::$logger->log(array(@date('[Y-m-d H-i-s]'),trim($message),self::$source?' @  '.self::$source:NULL,$exceptionFilename?' @@  '.basename($exceptionFilename):NULL),$priority);return$exceptionFilename?strtr($exceptionFilename,'\\/',DIRECTORY_SEPARATOR.DIRECTORY_SEPARATOR):NULL;}static
function
_shutdownHandler(){if(!self::$enabled){return;}static$types=array(E_ERROR=>1,E_CORE_ERROR=>1,E_COMPILE_ERROR=>1,E_PARSE=>1);$error=error_get_last();if(isset($types[$error['type']])){$exception=new
Nette\FatalErrorException($error['message'],0,$error['type'],$error['file'],$error['line'],NULL);if(PHP_VERSION_ID>=50300&&function_exists('xdebug_get_function_stack')){$stack=array();foreach(array_slice(array_reverse(xdebug_get_function_stack()),1,-1)as$row){$frame=array('file'=>$row['file'],'line'=>$row['line'],'function'=>isset($row['function'])?$row['function']:'*unknown*','args'=>array());if(!empty($row['class'])){$frame['type']=isset($row['type'])&&$row['type']==='dynamic'?'->':'::';$frame['class']=$row['class'];}$stack[]=$frame;}$ref=new\ReflectionProperty('Exception','trace');$ref->setAccessible(TRUE);$ref->setValue($exception,$stack);}self::_exceptionHandler($exception);}if(!connection_aborted()&&self::$bar&&!self::$productionMode&&self::isHtmlMode()){self::$bar->render();}}static
function
...
function
barDump($var,$title=NULL){if(!self::$productionMode){$dump=array();foreach((is_array($var)?$var:array(''=>$var))as$key=>$val){$dump[$key]=Dumper::toHtml($val);}self::$dumpPanel->data[]=array('title'=>$title,'dump'=>$dump);}return$var;}static
function
...
instanceof\Closure){$rc=new\ReflectionFunction($var);$fields=array();foreach($rc->getParameters()as$param){$fields[]='$'.$param->getName();}$fields=array('file'=>$rc->getFileName(),'line'=>$rc->getStartLine(),'parameters'=>implode(', ',$fields));}elseif($var
instanceof\SplFileInfo){$fields=array('path'=>$var->getPathname());}else{$fields=(array)$var;}static$list=array();$out='<span class="nette-dump-object">'.get_class($var)."</span> (".count($fields).')';if(empty($fields)){return$out."\n";}elseif(in_array($var,$list,TRUE)){return$out." { <i>RECURSION</i> }\n";}elseif(!$options[self::DEPTH]||$level<$options[self::DEPTH]||$var
...
static
function
findLocation(){foreach(debug_backtrace(FALSE)as$item){if(isset($item['file'])&&strpos($item['file'],__DIR__)===0){continue;}elseif(!isset($item['file'],$item['line'])||!is_file($item['file'])){break;}else{$lines=file($item['file']);$line=$lines[$item['line']-1];return
array($item['file'],$item['line'],preg_match('#\w*dump(er::\w+)?\(.*\)#i',$line,$m)?$m[0]:$line);}}}}class
Logger
extends
Nette\Object{const
...
instanceof
Nette\FatalErrorException&&isset($errorTypes[$exception->getSeverity()]))?$errorTypes[$exception->getSeverity()]:get_class($exception);$counter=0;?><!DOCTYPE html><!-- "' --></script></style></pre></xmp></table>
<html>
<head>
	<meta charset="utf-8">
	<meta name="robots" content="noindex">
...
2?'nette-alt':''?>">
	...
	<td><pre><?php echo
htmlspecialchars(Nette\Utils\Strings::fixEncoding($message)),' in ',Helpers::editorLink($file,$line)?></pre></td>
</tr>
<?php endforeach?>
</table>
</div>
...
instanceof
IResponse){$response->send($this->httpRequest,$this->httpResponse);}break;}catch(\Exception$e){$this->onError($this,$e);if($repeatedError){$e=new
ApplicationException("An error occurred while executing error-presenter '$this->errorPresenter'.",0,$e);}if($repeatedError||!$this->catchExceptions){$this->onShutdown($this,$e);throw$e;}$repeatedError=TRUE;$this->errorPresenter=$this->errorPresenter?:'Nette:Error';if(!$this->httpResponse->isSent()){$this->httpResponse->setCode($e
instanceof
BadRequestException?$e->getCode():500);}if($this->presenter
instanceof
...
Request($this->errorPresenter,Request::FORWARD,array('exception'=>$e));}}}while(1);$this->onShutdown($this,isset($e)?$e:NULL);}final
function
...
function
initializePanel(Nette\Application\Application$application){Nette\Diagnostics\Debugger::$blueScreen->addPanel(function($e)use($application){return$e?NULL:array('tab'=>'Nette Application','panel'=>'<h3>Requests</h3>'.Dumper::toHtml($application->getRequests()).'<h3>Presenter</h3>'.Dumper::toHtml($application->getPresenter()));});}function
...
createPresenter($name){$presenter=$this->container->createInstance($this->getPresenterClass($name));$this->container->callInjects($presenter);if($presenter
instanceof
...
as$module=>$mapping){$mapping=explode('\\\\\*',preg_quote($mapping,'#'));$mapping[0].=$mapping[0]?'\\\\':'';if(preg_match("#^\\\\?$mapping[0]((?:\\w+$mapping[1]\\\\)*)(\\w+)$mapping[2]\\z#i",$class,$matches)){return($module==='*'?'':$module.':').str_replace($mapping[1].'\\',':',$matches[1]).$matches[2];}}}function
...
templatePrepareFilters($template){$template->registerFilter($this->getPresenter()->getContext()->createNette__Latte());}function
...
validate(){foreach($this->getControls()as$control){$control->validate();}$this->onValidate($this);$this->validated=TRUE;}function
...
PROTECTOR_ID='_token_';public$onSuccess;public$onError;public$onSubmit;private$submittedBy;private$httpData;private$element;private$renderer;private$translator;private$groups=array();private$errors=array();function
...
instanceof
ISubmitterControl){if(!$this->submittedBy->getValidationScope()||$this->isValid()){$this->submittedBy->click();$valid=TRUE;}else{$this->submittedBy->onInvalidClick($this->submittedBy);}}if(isset($valid)||$this->isValid()){$this->onSuccess($this);}else{$this->onError($this);}$this->onSubmit($this);}protected
function
...
class
Framework{const
NAME='Nette Framework',VERSION='2.1-dev',REVISION='2725e26 released on 2013-03-14';public
...
as$handler){Callback::create($handler)->invokeArgs($args);}}elseif($_this->$name!==NULL){throw
new
UnexpectedValueException("Property $class::$$name must be array or NULL, ".gettype($_this->$name)." given.");}}elseif($cb=self::getExtensionMethod($class,$name)){array_unshift($args,$_this);return$cb->invokeArgs($args);}else{throw
new
...
new
MemberAccessException("Cannot read a class '$class' property without name.");}elseif(isset(self::$methods[$class][$m='get'.$uname])||isset(self::$methods[$class][$m='is'.$uname])){$isRef=&self::$methods[$class][$m];if(!is_bool($isRef)){$rm=new\ReflectionMethod($class,$m);$isRef=$rm->returnsReference();}if($isRef){return$_this->$m();}else{$val=$_this->$m();return$val;}}elseif(isset(self::$methods[$class][$name])){$val=Callback::create($_this,$name);return$val;}else{$type=isset(self::$methods[$class]['set'.$uname])?'a write-only':'an undeclared';throw
new
...
static
function
hasProperty($class,$name){$prop=&self::$props[$class][$name];if($prop===NULL){$prop=FALSE;try{$rp=new\ReflectionProperty($class,$name);if($name===$rp->getName()&&$rp->isPublic()&&!$rp->isStatic()){$prop=preg_match('#^on[A-Z]#',$name)?'event':TRUE;}}catch(\ReflectionException$e){}}return$prop;}static
function
setExtensionMethod($class,$name,$callback){$l=&self::$extMethods[strtolower($name)];$l[strtolower($class)]=new
Callback($callback);$l['']=NULL;}static
function
getExtensionMethod($class,$name){$class=strtolower($class);$l=&self::$extMethods[strtolower($name)];if(empty($l)){return
FALSE;}elseif(isset($l[''][$class])){return$l[''][$class];}$cl=$class;do{if(isset($l[$cl])){return$l[''][$class]=$l[$cl];}}while(($cl=strtolower(get_parent_class($cl)))!=='');foreach(class_implements($class)as$cl){$cl=strtolower($cl);if(isset($l[$cl])){return$l[''][$class]=$l[$cl];}}return$l[''][$class]=FALSE;}}}namespace Nette\ComponentModel{use
Nette;class
...
dump(array$data){return"<?php // generated by Nette \nreturn ".Nette\PhpGenerator\Helpers::dump($data).';';}}}namespace Nette\Config{use
Nette;use
...
new
Nette\DI\ServiceCreationException("Service '$name': ".$e->getMessage(),NULL,$e);}if($definition->class==='self'){$definition->class=$origName;}if($definition->factory&&$definition->factory->entity==='self'){$definition->factory->entity=$origName;}}}static
function
parseService(Nette\DI\ServiceDefinition$definition,$config,$shared=TRUE){if($config===NULL){return;}elseif(!$shared&&is_string($config)&&interface_exists($config)){$config=array('class'=>NULL,'implement'=>$config);}elseif(!$shared&&$config
...
afterCompile(Nette\PhpGenerator\ClassType$class){}}use
Nette\Caching\Cache;class
Configurator
extends
Nette\Object{const
...
Extensions\NetteExtension)->addExtension('extensions',new
Extensions\ExtensionsExtension);return$compiler;}protected
function
createLoader(){return
new
Loader;}protected
...
__call($name,$args){if(substr($name,0,6)==='create'){$method=$this->container->getMethodName('nette.'.substr($name,6),FALSE);trigger_error("Factory accessing via nette->$name() is deprecated, use $method().",E_USER_DEPRECATED);return
...
NetteExtension
extends
Nette\Config\CompilerExtension{public$defaults=array('session'=>array('debugger'=>FALSE,'iAmUsingBadHost'=>NULL,'autoStart'=>'smart','expiration'=>NULL),'application'=>array('debugger'=>TRUE,'errorPresenter'=>NULL,'catchExceptions'=>'%productionMode%','mapping'=>NULL),'routing'=>array('debugger'=>TRUE,'routes'=>array()),'security'=>array('debugger'=>TRUE,'frames'=>'SAMEORIGIN','users'=>array(),'roles'=>array(),'resources'=>array()),'mailer'=>array('smtp'=>FALSE),'database'=>array(),'forms'=>array('messages'=>array()),'latte'=>array('xhtml'=>TRUE,'macros'=>array()),'container'=>array('debugger'=>FALSE),'debugger'=>array('email'=>NULL,'editor'=>NULL,'browser'=>NULL,'strictMode'=>NULL,'bar'=>array(),'blueScreen'=>array()));public$databaseDefaults=array('dsn'=>NULL,'user'=>NULL,'password'=>NULL,'options'=>NULL,'debugger'=>TRUE,'explain'=>TRUE,'reflection'=>'Nette\Database\Reflection\DiscoveredReflection');function
loadConfiguration(){$container=$this->getContainerBuilder();$config=$this->getConfig($this->defaults);if(isset($config['xhtml'])){$config['latte']['xhtml']=$config['xhtml'];}$container->addDefinition('nette')->setClass('Nette\Config\Extensions\NetteAccessor',array('@container'));$this->setupCache($container);$this->setupHttp($container);$this->setupSession($container,$config['session']);$this->setupSecurity($container,$config['security']);$this->setupApplication($container,$config['application']);$this->setupRouting($container,$config['routing']);$this->setupMailer($container,$config['mailer']);$this->setupForms($container);$this->setupTemplating($container,$config['latte']);$this->setupDatabase($container,$config['database']);}private
function
...
function
setupHttp(ContainerBuilder$container){$container->addDefinition($this->prefix('httpRequestFactory'))->setClass('Nette\Http\RequestFactory')->addSetup('setEncoding',array('UTF-8'));$container->addDefinition('httpRequest')->setClass('Nette\Http\Request')->setFactory('@Nette\Http\RequestFactory::createHttpRequest');$container->addDefinition('httpResponse')->setClass('Nette\Http\Response');$container->addDefinition($this->prefix('httpContext'))->setClass('Nette\Http\Context');}private
function
setupSession(ContainerBuilder$container,array$config){$session=$container->addDefinition('session')->setClass('Nette\Http\Session');if(isset($config['expiration'])){$session->addSetup('setExpiration',array($config['expiration']));}if(isset($config['iAmUsingBadHost'])){$session->addSetup('Nette\Framework::$iAmUsingBadHost = ?;',array((bool)$config['iAmUsingBadHost']));}if($container->parameters['debugMode']&&$config['debugger']){$session->addSetup('Nette\Diagnostics\Debugger::$bar->addPanel(?)',array(new
Nette\DI\Statement('Nette\Http\Diagnostics\SessionPanel')));}unset($config['expiration'],$config['autoStart'],$config['iAmUsingBadHost'],$config['debugger']);if(!empty($config)){$session->addSetup('setOptions',array($config));}}private
function
...
as$name=>$info){if(!is_array($info)){continue;}$info+=$this->databaseDefaults+array('autowired'=>$autowired);$autowired=FALSE;foreach((array)$info['options']as$key=>$value){unset($info['options'][$key]);$info['options'][constant($key)]=$value;}if(!$info['reflection']){$reflection=NULL;}elseif(is_string($info['reflection'])){$reflection=new
Nette\DI\Statement(preg_match('#^[a-z]+\z#',$info['reflection'])?'Nette\Database\Reflection\\'.ucfirst($info['reflection']).'Reflection':$info['reflection'],strtolower($info['reflection'])==='discovered'?array('@self'):array());}else{$tmp=Nette\Config\Compiler::filterArguments(array($info['reflection']));$reflection=reset($tmp);}$connection=$container->addDefinition($this->prefix("database.$name"))->setClass('Nette\Database\Connection',array($info['dsn'],$info['user'],$info['password'],$info['options']))->setAutowired($info['autowired'])->addSetup('setSelectionFactory',array(new
Nette\DI\Statement('Nette\Database\Table\SelectionFactory',array('@self',$reflection))))->addSetup('Nette\Diagnostics\Debugger::$blueScreen->addPanel(?)',array('Nette\Database\Diagnostics\ConnectionPanel::renderException'));if($container->parameters['debugMode']&&$info['debugger']){$connection->addSetup('Nette\Database\Helpers::createDebugPanel',array($connection,!empty($info['explain'])));}}}function
...
queryArgs($statement,array$params){if($params){list($statement,$params)=$this->preprocessor->process($statement,$params);}return$this->prepare($statement)->execute($params);}function
...
table($table){if(!$this->selectionFactory){$this->selectionFactory=new
Table\SelectionFactory($this);}return$this->selectionFactory->create($table);}function
setSelectionFactory(Table\SelectionFactory$selectionFactory){$this->selectionFactory=$selectionFactory;return$this;}function
...
setCacheStorage(){trigger_error(__METHOD__.'() is deprecated; use setSelectionFactory() instead.',E_USER_DEPRECATED);}static
function
getReflection(){return
new
...
as$i=>$query){list($sql,$params,$time,$rows,$connection,$source)=$query;$explain=NULL;if($this->explain&&preg_match('#\s*\(?\s*SELECT\s#iA',$sql)){try{$cmd=is_string($this->explain)?$this->explain:'EXPLAIN';$explain=$connection->queryArgs("$cmd $sql",$params)->fetchAll();}catch(\PDOException$e){}}$s.='<tr><td>'.sprintf('%0.3f',$time*1000);if($explain){static$counter;$counter++;$s.="<br /><a class='nette-toggle-collapsed' href='#nette-DbConnectionPanel-row-$counter'>explain</a>";}$s.='</td><td class="nette-DbConnectionPanel-sql">'.Helpers::dumpSql($sql,$params);if($explain){$s.="<table id='nette-DbConnectionPanel-row-$counter' class='nette-collapsed'><tr>";foreach($explain[0]as$col=>$foo){$s.="<th>{$h($col)}</th>";}$s.="</tr>";foreach($explain
as$row){$s.="<tr>";foreach($row
as$col){$s.="<td>{$h($col)}</td>";}$s.="</tr>";}$s.="</table>";}if($source){$s.=Nette\Diagnostics\Helpers::editorLink($source[0],$source[1])->class('nette-DbConnectionPanel-source');}$s.='</td><td>'.$rows.'</td></tr>';}return
...
new
Nette\FileNotFoundException("Cannot open file '$file'.");}$count=0;$sql='';while(!feof($handle)){$s=fgets($handle);$sql.=$s;if(substr(rtrim($s),-1)===';'){$connection->exec($sql);$sql='';$count++;}}if(trim($sql)!==''){$connection->exec($sql);$count++;}fclose($handle);return$count;}static
function
createDebugPanel($connection,$explain=TRUE){$panel=new
Nette\Database\Diagnostics\ConnectionPanel($connection);$panel->explain=$explain;Nette\Diagnostics\Debugger::$bar->addPanel($panel);return$panel;}}}namespace Nette\Database\Reflection{use
Nette;class
...
extends
Nette\Object
implements\Iterator,\ArrayAccess,\Countable{protected$connection;protected$reflection;protected$cache;protected$sqlBuilder;protected$name;protected$primary;protected$primarySequence=FALSE;protected$rows;protected$data;protected$dataRefreshed=FALSE;protected$referenced=array();protected$referencing=array();protected$referencingPrototype=array();protected$aggregation=array();protected$accessedColumns;protected$previousAccessedColumns;protected$observeCache=FALSE;protected$checkReferenced=FALSE;protected$keys=array();function
...
SqlBuilder($table,$connection,$reflection);}function
...
__construct(Selection$refTable,$table,$column){parent::__construct($refTable->connection,$table,$refTable->reflection,$refTable->cache?$refTable->cache->getStorage():NULL);$this->refTable=$refTable;$this->column=$column;}function
...
__construct(Nette\Database\Connection$connection,Nette\Database\IReflection$reflection=NULL,Nette\Caching\IStorage$cacheStorage=NULL){$this->connection=$connection;$this->reflection=$reflection?:new
Nette\Database\Reflection\ConventionalReflection;$this->cacheStorage=$cacheStorage;}function
create($table){return
new
Selection($this->connection,$table,$this->reflection,$this->cacheStorage);}}use
...
extends
Nette\FreezableObject{const
TAGS='tags';public$parameters=array();public$classes=array();private$registry=array();private$factories=array();public$meta=array();private$creating;function
...
Nette\PhpGenerator\Helpers as PhpHelpers;use
Nette\PhpGenerator\PhpLiteral;class
ContainerBuilder
extends
Nette\Object{const
THIS_SERVICE='self',THIS_CONTAINER='container';public$parameters=array();private$definitions=array();private$classes;private$dependencies=array();private$generatedClasses=array();function
...
new
Nette\InvalidStateException("Interface $def->implement has not been found.");}$rc=Reflection\ClassType::from($def->implement);$method=$rc->hasMethod('create')?$rc->getMethod('create'):($rc->hasMethod('get')?$rc->getMethod('get'):NULL);if(count($rc->getMethods())!==1||!$method||$method->isStatic()){throw
new
Nette\InvalidStateException("Interface $def->implement must have just one non-static method create() or get().");}$def->implement=$rc->getName();if(!$def->class&&empty($def->factory->entity)){$returnType=$method->getAnnotation('return');if(!$returnType){throw
new
Nette\InvalidStateException("Method $method has not @return annotation.");}if(!class_exists($returnType)){if($returnType[0]!=='\\'){$returnType=$rc->getNamespaceName().'\\'.$returnType;}if(!class_exists($returnType)){throw
new
Nette\InvalidStateException("Please use a fully qualified name of class in @return annotation at $method method. Class '$returnType' cannot be found.");}}$def->setClass($returnType);}if($method->getName()==='get'){if($method->getParameters()){throw
new
Nette\InvalidStateException("Method $method must have no arguments.");}if(empty($def->factory->entity)){$def->setFactory('@\\'.ltrim($def->class,'\\'));}elseif(!$this->getServiceName($def->factory->entity)){throw
new
Nette\InvalidStateException("Invalid factory in service '$name' definition.");}}if(!$def->parameters){foreach($method->getParameters()as$param){$paramDef=($param->isArray()?'array':$param->getClassName()).' '.$param->getName();if($param->isOptional()){$def->parameters[$paramDef]=$param->getDefaultValue();}else{$def->parameters[]=$paramDef;}}}}foreach($this->definitions
as$name=>$def){if($def->class){$def->class=$this->expand($def->class);if(!$def->factory){$def->factory=new
...
as$name=>$def){$factory=$this->normalizeEntity($this->expand($def->factory->entity));if(is_string($factory)&&preg_match('#^[\w\\\\]+\z#',$factory)&&$factory!==self::THIS_SERVICE){if(!class_exists($factory)||!Reflection\ClassType::from($factory)->isInstantiable()){throw
new
Nette\InvalidStateException("Class $factory used in service '$name' has not been found or is not instantiable.");}}}foreach($this->definitions
as$name=>$def){$this->resolveClass($name);if(!$def->class){continue;}elseif(!class_exists($def->class)&&!interface_exists($def->class)){throw
new
Nette\InvalidStateException("Class or interface $def->class used in service '$name' has not been found.");}else{$def->class=Reflection\ClassType::from($def->class)->getName();}}$this->classes=array();foreach($this->definitions
...
new
ServiceCreationException("Unable resolve class name for service '$name'.");}$factory[0]=$this->resolveClass($service,$recursive);if(!$factory[0]){return;}if($this->definitions[$service]->implement&&$factory[1]==='create'){return$def->class=$factory[0];}}$factory=new
...
Nette\PhpGenerator\ClassType('Container');$containerClass->addExtend('Nette\DI\Container');$containerClass->addMethod('__construct')->addBody('parent::__construct(?);',array($this->expand($this->parameters)));$prop=$containerClass->addProperty('classes',array());foreach($this->classes
as$name=>$foo){try{$prop->value[$name]=$this->getByType($name);}catch(ServiceCreationException$e){$prop->value[$name]=new
PhpLiteral('FALSE, //'.strstr($e->getMessage(),':'));}}$definitions=$this->definitions;ksort($definitions);$meta=$containerClass->addProperty('meta',array());foreach($definitions
...
as$setup){$setup=Helpers::expand($setup,$parameters,TRUE);if(is_string($setup->entity)&&strpbrk($setup->entity,':@?')===FALSE){$setup->entity=array('@self',$setup->entity);}$code.=$this->formatStatement($setup,$name).";\n";}$code.='return $service;';if(!$def->implement){return$code;}$factoryClass=$this->generatedClasses[]=new
...
function
convertParameters(array$parameters){$res=array();foreach($this->expand($parameters)as$k=>$v){$tmp=explode(' ',is_int($k)?$v:$k);$param=$res[]=new
...
PhpLiteral($val);}elseif(is_string($val)&&preg_match('#^[\w\\\\]*::[A-Z][A-Z0-9_]*\z#',$val,$m)){$val=new
PhpLiteral(ltrim($val,':'));}});return
...
FALSE;}$service=substr($arg,1);if($service===self::THIS_SERVICE){$service=$self;}if(Strings::contains($service,'\\')){if($this->classes===FALSE){return$service;}$res=$this->getByType($service);if(!$res){throw
new
...
Dumper::toHtml($container->meta[$name],array(Dumper::COLLAPSE=>TRUE));}?></td>
		</tr>
		<?php endforeach?>
		</tbody>
	</table>
...
new
Nette\InvalidStateException("Property $property has not @var annotation.");}elseif(!class_exists($type)){if($type[0]!=='\\'){$type=$class->getNamespaceName().'\\'.$type;}if(!class_exists($type)){throw
new
Nette\InvalidStateException("Please use a fully qualified name of class in @var annotation at $property property. Class '$type' cannot be found.");}}$res[$property->getName()]=$type;}return$res;}}class
ServiceDefinition
extends
...
getControl(){$this->setOption('rendered',TRUE);$control=clone$this->control;$control->name=$this->getHtmlName();$control->disabled=$this->disabled;$control->id=$this->getHtmlId();$control->required=$this->isRequired();if(isset($control->placeholder)){$control->placeholder=$this->translate($control->placeholder);}$rules=self::exportRules($this->rules);$rules=substr(Nette\Utils\Json::encode($rules),1,-1);$rules=preg_replace('#"([a-z0-9_]+)":#i','$1:',$rules);$rules=preg_replace('#(?<!\\\\)"(?!:[^a-z])([^\\\\\',]*)"#i',"'$1'",$rules);$control->data('nette-rules',$rules?$rules:NULL);return$control;}function
...
instanceof
IControl?(object)array('control'=>$rule->arg->getHtmlName()):$rule->arg;}$payload[]=$item;}return$payload;}function
validate(){$this->errors=$this->rules->validate();}static
function
...
function
validateValid(IControl$control){return!$control->rules->validate();}static
function
...
setValue($value){parent::setValue($value);if(is_array($value)&&isset($value[0],$value[1])){$this->value=$value;}return$this;}function
getHtmlName(){return
parent::getHtmlName().'[]';}}class
SelectBox
extends
...
setPrompt($prompt){if($prompt===TRUE){trigger_error(__METHOD__.'(TRUE) is deprecated; argument must be string.',E_USER_DEPRECATED);$prompt=reset($this->items);unset($this->allowed[key($this->items)],$this->items[key($this->items)]);}$this->prompt=$prompt;return$this;}final
function
...
SelectBox{function
getValue(){return
array_values(array_intersect($this->getRawValue(),array_keys($this->allowed)));}function
...
render(Nette\Forms\Form$form,$mode=NULL){if($this->form!==$form){$this->form=$form;$this->init();}$s='';if(!$mode||$mode==='begin'){$s.=$this->renderBegin();}if(!$mode||$mode==='errors'){$s.=$this->renderErrors();}if(!$mode||$mode==='body'){$s.=$this->renderBody();}if(!$mode||$mode==='end'){$s.=$this->renderEnd();}return$s;}protected
function
...
instanceof
Html){$description=' '.$control->getOption('description');}elseif(is_string($description)){$description=' '.$this->getWrapper('control description')->setText($control->translate($description));}else{$description='';}if($control->isRequired()){$description=$this->getValue('control requiredsuffix').$description;}$description.=$this->renderErrors($control);if($control
instanceof
Nette\Forms\Controls\Checkbox||$control
instanceof
...
validate(){$errors=array();foreach($this->rules
as$rule){if($rule->control->isDisabled()){continue;}$success=($rule->isNegative
xor$this->getCallback($rule)->invoke($rule->control,$rule->arg));if($rule->type===Rule::CONDITION&&$success){if($tmp=$rule->subRules->validate()){$errors=array_merge($errors,$tmp);break;}}elseif($rule->type===Rule::VALIDATOR&&!$success){$errors[]=static::formatMessage($rule,TRUE);break;}}return$errors;}final
function
getIterator(){return
...
ob_get_clean();}function
getPanel(){ob_start();?>
<style class="nette-debug">#nette-debug .nette-SessionPanel .nette-inner{width:700px}</style>
...
appendQuery($value){$this->updating();$value=(string)(is_array($value)?http_build_query($value,'','&'):$value);$this->query.=($this->query===''||$value==='')?$value:'&'.$value;return$this;}function
...
canonicalize(){$this->updating();$this->path=$this->path===''?'/':self::unescape($this->path,'%/');$this->host=strtolower(rawurldecode($this->host));$this->query=self::unescape(strtr($this->query,'+',' '),'%&;=+');return$this;}function
...
accept(){return$this->callback->invoke($this->current(),$this->key(),$this);}}class
Mapper
...
as$item){$node=$this->writeMacro($item[0],$item[1],NULL,NULL,$item[2]);if($node->closing){$this->htmlNode->attrCode.=$node->attrCode;}}if($right&&substr($this->output,-2)==='?>'){$this->output.="\n";}}function
...
macroControl(MacroNode$node,PhpWriter$writer){if($node->name==='widget'){trigger_error('Macro {widget} is deprecated; use {control} instead.',E_USER_DEPRECATED);}$pair=$node->tokenizer->fetchWord();if($pair===FALSE){throw
new
...
new
Nette\InvalidArgumentException('Template is not valid UTF-8 stream.');}$input=str_replace("\r\n","\n",$input);$this->input=$input;$this->output=array();$this->offset=0;$this->setSyntax($this->defaultSyntax);$this->setContext(self::CONTEXT_HTML_TEXT);$this->lastHtmlTag=$this->syntaxEndTag=NULL;while($this->offset<strlen($input)){$matches=$this->{"context".$this->context[0]}();if(!$matches){break;}elseif(!empty($matches['comment'])){$this->addToken(Token::COMMENT,$matches[0]);}elseif(!empty($matches['macro'])){$token=$this->addToken(Token::MACRO_TAG,$matches[0]);list($token->name,$token->value,$token->modifiers)=$this->parseMacroTag($matches['macro']);}$this->filter();}if($this->offset<strlen($input)){$this->addToken(Token::TEXT,substr($this->input,$this->offset));}return$this->output;}private
function
...
FALSE;}if($match['name']===''){$match['name']=$match['shortname']?:'=';if(!$match['noescape']&&substr($match['shortname'],0,1)!=='/'&&$match['shortname']!=='#'){$match['modifiers'].='|escape';}}return
...
Compiler::CONTEXT_UNQUOTED_ATTR:if($context[1]===Compiler::CONTENT_JS){$s="Nette\\Templating\\Helpers::escapeJs($s)";}elseif($context[1]===Compiler::CONTENT_CSS){$s="Nette\\Templating\\Helpers::escapeCss($s)";}$quote=$context[0]===Compiler::CONTEXT_SINGLE_QUOTED_ATTR?', ENT_QUOTES':'';$s="htmlSpecialChars($s$quote)";return$context[0]===Compiler::CONTEXT_UNQUOTED_ATTR?"'\"' . $s . '\"'":$s;case
...
register($prepend=FALSE){$this->classes=$this->getCache()->load($this->getKey(),new
Nette\Callback($this,'_rebuildCallback'));parent::register($prepend);return$this;}function
...
ClassType
extends
Nette\Object{public$name;public$type='class';public$final;public$abstract;public$extends=array();public$implements=array();public$traits=array();public$documents=array();public$consts=array();public$properties=array();public$methods=array();static
function
from($from){$from=$from
...
as$k=>&$v){if($k!==$marker){$out.="$space\t".($k===$counter?'':self::_dump($k,$level+1)." => ").self::_dump($v,$level+1).",\n";$counter=is_int($k)?max($k+1,$counter):$counter;}}unset($var[$marker]);$out.=$space;}return"array($out)";}elseif(is_object($var)){$arr=(array)$var;$space=str_repeat("\t",$level);static$list=array();if(empty($arr)){$out='';}elseif($level>50||in_array($var,$list,TRUE)){throw
new
...
Method
extends
Nette\Object{public$name;public$parameters=array();public$uses=array();public$body;public$static;public$visibility;public$final;public$abstract;public$returnReference;public$documents=array();static
function
from($from){$from=$from
...
Parameter
extends
Nette\Object{public$name;public$reference;public$typeHint;public$optional;public$defaultValue;static
function
...
Property
extends
Nette\Object{public$name;public$value;public$static;public$visibility='public';public$documents=array();static
function
...
__call($name,$args){return
Nette\ObjectMixin::callProperty($this,$name,$args);}}}namespace Nette\Reflection{use
Nette;class
Annotation
extends
Nette\Object
...
function
getAuthorizator(){return$this->authorizator?:$this->context->getByType('Nette\Security\IAuthorizator');}}}namespace Nette\Templating{use
Nette;use
Nette\Caching;class
Template
extends
...
registerFilter($callback){$this->filters[]=new
...
function
isList($value){return
is_array($value)&&(!$value||array_keys($value)===range(0,count($value)-1));}}use
...
new
JsonException($message);});$json=call_user_func_array('json_encode',$args);restore_error_handler();if(isset($old)){ini_set('display_errors',$old);}if(PHP_VERSION_ID>=50300&&($error=json_last_error())){throw
new
...
function
decode($json,$options=0){$json=(string)$json;$args=array($json,(bool)($options&self::FORCE_ARRAY));if(PHP_VERSION_ID>=50300){$args[]=512;if(PHP_VERSION_ID>=50400){$args[]=JSON_BIGINT_AS_STRING;}}$value=call_user_func_array('json_decode',$args);if($value===NULL&&$json!==''&&strcasecmp($json,'null')){$error=PHP_VERSION_ID>=50300?json_last_error():0;throw
new
...
debug(){Debugger::$strictMode=TRUE;Debugger::enable(Debugger::DEVELOPMENT);}function
